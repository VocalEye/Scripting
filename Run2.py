...
def getCapturer():
    capture = False
    while(not capture):
        # capture = cv2.VideoCapture("./input/videos/TEST5.mp4")
...
def predictEye(image, innerMargin):
    firstPoint, secondPoint = findMarginOfEye(innerMargin)
    image = cropImageAround(image, firstPoint, secondPoint)
    parsed = CustomParser(image)
    return MODEL.predict(parsed, verbose = 0)
    ...
    with mp_face_mesh.FaceMesh(
        ...
        while capturer.isOpened():
            ...
            if results.multi_face_landmarks:
                ...
                rightArray = predictEye(full_image, mesh_points[RIGHT_EYE])
                leftArray = predictEye(full_image, mesh_points[LEFT_EYE])
                averageArray = [(x + y)/2 for x, y in zip(rightArray, leftArray)]
                
                rightPrediction = getPredictedClass(rightArray)[0] + 1
                leftPrediction = getPredictedClass(leftArray)[0] + 1
                averagePrediction = getPredictedClass(averageArray)[0] + 1
                
            full_image = cv2.putText(full_image, "Derecho: " + fetchCharacter(rightPrediction), (100, 150), cv2.FONT_HERSHEY_SIMPLEX, 
                   2, (255, 0, 0), 2, cv2.LINE_AA)
            full_image = cv2.putText(full_image, "Izquierdo: " + fetchCharacter(leftPrediction), (100, 200), cv2.FONT_HERSHEY_SIMPLEX, 
                   2, (255, 0, 0), 2, cv2.LINE_AA)
            
            full_image = cv2.putText(full_image, "Promedio: " + fetchCharacter(averagePrediction), (100, 250), cv2.FONT_HERSHEY_SIMPLEX, 
                   2, (255, 0, 0), 2, cv2.LINE_AA)
